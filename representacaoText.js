function verLista(lista, quantVertice, nome, nomes){
  ...
  for(var i = 0; i < quantVertice; i++){
    ...
    html += '<div class="com">'+ nomes[i] + '</div>'

    for(var j = 0; lista[i][j] != undefined ; j++){
      html += '<div class="sem">➝</div>'
      html += '<div class="com">' + nomes[j]+ ' - Peso(' + lista[i][j].peso +')</div>'
    }
    html += '</div>'
  }
  html += '</div>'
...
function verBuscaProfundidadeLista(lista, quantVertice, verticeEscolhido,nomes){
  var html = '<p class="title">Busca em profundidade a partir do vértice ' + verticeEscolhido + ' - Lista de Adjacência</p>'
  html += '<div class="representacao " >'
  html += '<table class="table table-bordered">'
  html += '<thead>'
  html += '<th></th>'
  for(var i = 0; i < quantVertice; i++) html += '<th>Vértice ' + nomes[i] + '</th>'
  html += '</thead><tbody>'
...
function verBuscaLargura(lista, quantVertice, verticeEscolhido,nomes){
  
  var html = '<p class="title">Busca em largura a partir do vértice ' + verticeEscolhido + '</p>'
  html += '<div class="representacao ">'
  for(var i = 0; i < quantVertice; i++){
    html += '<div class="row " style="margin-bottom: 10px;">'
    html += '<div class="com">De '+ nomes[verticeEscolhido] + ' Para ' + nomes[i] +  '</div>'
...
function verCaminhoMin(cm, quantVertice, vertice,nomes){
  var html = '<div class="representacao ">'
  ...
  for(var i = 0; i < quantVertice; i++){
    ...
    if(i != vertice){
      html += '<div class="row " style="margin-bottom: 10px;">'
      html += '<div class="com">De ' + nomes[vertice] +' para ' + nomes[i] +'</div>'
      var j = i
      var aux = ''
      var pilha = []
      var trix = 0
